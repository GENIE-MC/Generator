...
/*
 ...
 @ Oct 05, 2009 - CA
   ...
   Also, the helicity amplitude code now returns a `const MAIDHelicityAmpl &'.
 @ July 23, 2010 - CA
...
#include "eresMAID/eresMAIDRESPXSec.h"
...
//____________________________________________________________________________
eresMAIDRESPXSec::eresMAIDRESPXSec() :
XSecAlgorithmI("genie::eresMAIDRESPXSec")
{
  fNuTauRdSpl    = 0;
  fNuTauBarRdSpl = 0;
}
//____________________________________________________________________________
eresMAIDRESPXSec::eresMAIDRESPXSec(string config) :
XSecAlgorithmI("genie::eresMAIDRESPXSec", config)
{
  fNuTauRdSpl    = 0;
  fNuTauBarRdSpl = 0;
}
                 ...
                 const Interaction * interaction, KinePhaseSpace_t kps) const
{
  ...
  const MAIDHelicityAmplModelI * hamplmod = 0;
  if(is_CC) { 
  ...
  const MAIDHelicityAmpl & hampl = hamplmod->Compute(resonance, fFKR, interaction); 
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("eresMAIDRes", pDEBUG) << "sig_{0} = " << sig0;
  LOG("eresMAIDRes", pDEBUG) << "sig_{L} = " << sigL;
  LOG("eresMAIDRes", pDEBUG) << "sig_{R} = " << sigR;
  LOG("eresMAIDRes", pDEBUG) << "sig_{S} = " << sigS;
#endif
...
//____________________________________________________________________________
double eresMAIDRESPXSec::Integral(const Interaction * interaction) const
{
  double xsec = fXSecIntegrator->Integrate(this,interaction);
  return xsec;
}
//____________________________________________________________________________
bool eresMAIDRESPXSec::ValidProcess(const Interaction * interaction) const
{
...
//____________________________________________________________________________
void eresMAIDRESPXSec::Configure(const Registry & config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void eresMAIDRESPXSec::Configure(string config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void eresMAIDRESPXSec::LoadConfig(void)
{
  ...
  fHAmplModelCC  = dynamic_cast<const MAIDHelicityAmplModelI *> (
      algf->GetAlgorithm("genie::MAIDHelicityAmplModelCC","Default"));
  fHAmplModelNCp = dynamic_cast<const MAIDHelicityAmplModelI *> (
      algf->GetAlgorithm("genie::MAIDHelicityAmplModelNCp","Default"));
  fHAmplModelNCn = dynamic_cast<const MAIDHelicityAmplModelI *> (
      algf->GetAlgorithm("genie::MAIDHelicityAmplModelNCn","Default"));
  fHAmplModelEMp = dynamic_cast<const MAIDHelicityAmplModelI *> (
      algf->GetAlgorithm("genie::MAIDHelicityAmplModelEMp","Default"));
  fHAmplModelEMn = dynamic_cast<const MAIDHelicityAmplModelI *> (
      algf->GetAlgorithm("genie::MAIDHelicityAmplModelEMn","Default"));
