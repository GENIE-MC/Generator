...
\brief    Computes the cross section for an exclusive 1pi reaction through
          ...
          specifying the genie::eresMAIDSPPPXSec as the differential 
          cross section model. However, eresMAIDSPPXSec offers a faster 
...
#ifndef _ERESMAID_SPP_XSEC_H_
#define _ERESMAID_SPP_XSEC_H_

#include "eresMAID/eresMAIDRESXSecWithCache.h"

namespace genie {

class eresMAIDSPPXSec : public eresMAIDRESXSecWithCache {

public:
  eresMAIDSPPXSec();
  eresMAIDSPPXSec(string param_set);
  virtual ~eresMAIDSPPXSec();
