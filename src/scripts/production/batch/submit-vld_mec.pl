...
# 18081Mxxx      | numubar  + H2O                | CCMEC+CCQE    | T2K/SK numubar
...
%evg_tgtpdg_hash = ( 
  ...
  '18081' =>   '1000080160[0.889],1000010010[0.111]',
...
%evg_fluxopt_hash = ( 
  ...
  '18081' =>   '-f $flux_dir/t2ksk.root[numubar_flux]',
  '19090' =>   '-f $flux_dir/x'
);
...
for my $curr_runnu (keys %evg_gevgl_hash)  {
  ...
  if($runnu=~m/$curr_runnu/ || $runnu eq "all") {
    ...
    for($isubrun = 0; $isubrun < $nsubruns; $isubrun++) {
       ...
       $evgen_cmd     = "gevgen -n $nev_per_subrun -e $en -p $probe -t $tgt -r $curr_subrunnu $fluxopt | $grep_pipe &> $fntemplate.evgen.log";
