...
# - Using standard SK statistics: 500 numu, 50 numubar, 50 nue, 50 nuesig files (2k events each, see submit-evg_sk_fhst.pl)
...
perl bulk-submit.pl --cmd "perl submit-evg_sk_fhst.pl --neutrino numubar --version $VRS --production $PRO --cycle $CYC" --arg "--run" --min 1 --max 50
perl bulk-submit.pl --cmd "perl submit-evg_sk_fhst.pl --neutrino nue     --version $VRS --production $PRO --cycle $CYC" --arg "--run" --min 1 --max 50
perl bulk-submit.pl --cmd "perl submit-evg_sk_fhst.pl --neutrino nuesig  --version $VRS --production $PRO --cycle $CYC" --arg "--run" --min 1 --max 50
