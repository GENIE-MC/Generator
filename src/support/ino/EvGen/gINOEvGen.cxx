...
\brief   A GENIE event generation driver 'customized' for INO.
         ...
         *** Examples:
           ...
           (1) Generate 10k events (run number 999210) in the energy range 1-10 GeV
               ...
               % gINOevgen -r 999210 -n 10000 -e 1,10
...
void GetCommandLineArgs(int argc, char ** argv)
{
  ...
  // print out
  LOG("gINOevgen", pNOTICE) << "Job configuration:";
  LOG("gINOevgen", pNOTICE) << "Number of events = " << gOptNev;
  LOG("gINOevgen", pNOTICE) << "Emin = " << gOptEvMin;
  LOG("gINOevgen", pNOTICE) << "Emax = " << gOptEvMax;
  map<int,string>::const_iterator file_iter = gOptFluxFiles.begin();
  for( ; file_iter != gOptFluxFiles.end(); ++file_iter) {
     int neutrino_code = file_iter->first;
     string filename   = file_iter->second;
  ...
  LOG("gINOevgen", pNOTICE) << "Done reading command line arguments";
}
//________________________________________________________________________________________
void PrintSyntax(void)
{
  LOG("gINOevgen", pFATAL) 
   << "\n **Syntax**"
   << "\n gINOevgen [-h] "
   << "\n           [-r run#]"
