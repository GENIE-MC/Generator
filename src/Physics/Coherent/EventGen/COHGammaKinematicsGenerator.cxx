...
void COHGammaKinematicsGenerator::ProcessEventRecord(GHepRecord *evrec) const {
  ...
  if (ftPhaseSpace)
    func = new utils::gsl::d4Xsec_dEgdtdThetagdPhig(fXSecModel,
                                                    &local_interaction);
  else
    func = new utils::gsl::d4Xsec_dEgdThetaldThetagdPhig(fXSecModel,
                                                         &local_interaction);
...
COHGammaKinematicsGenerator::ComputeMaxXSec(const Interaction *in) const {
  ...
  std::array<string, 4> names = {"E_g", ftPhaseSpace ? "t" : "theta_l",
                                 "theta_g", "phi_g"};
...
//___________________________________________________________________________
void COHGammaKinematicsGenerator::throwOnTooManyIterations(
