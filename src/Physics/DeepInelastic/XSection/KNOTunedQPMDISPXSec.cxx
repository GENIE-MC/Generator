...
//____________________________________________________________________________
double
KNOTunedQPMDISPXSec::DISRESJoinSuppressionFactor(const Interaction *in) const {
  ...
  if (!fUseCache) {
    ...
    if (!cbr) {
      ...
      double WminSpl = Wmin;
      double WmaxSpl =
          fWcut + 0.1; // well into the area where scaling factor = 1
      double dW = (WmaxSpl - WminSpl) / (kN - 1);

      for (int i = 0; i < kN; i++) {
        double W = WminSpl + i * dW;
        interaction.KinePtr()->SetW(W);
        mprob = fHadronizationModel->MultiplicityProb(&interaction,
                                                      "+LowMultSuppr");
        R = 1;
        if (mprob) {
          R = mprob->Integral("width");
  ...
  if (Wo > Wmin && Wo < fWcut) {
    Ro = R;
    if (is_EM)
      Ro *= fNRBEMScale; // Additional scaling
  } else if (Wo <= Wmin)
    Ro = 0.0;
  else
    Ro = 1.0;
...
void KNOTunedQPMDISPXSec::LoadConfig(void) {
  ...
  // Here we provide the option to turn the caching off at run-time (default:
  // on)
