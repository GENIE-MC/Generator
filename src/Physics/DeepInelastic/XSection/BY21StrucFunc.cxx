...
//____________________________________________________________________________
BY21StrucFunc::BY21StrucFunc() : QPMDISStrucFuncBase("genie::BY21StrucFunc") {
  this->Init();
}
//____________________________________________________________________________
...
//____________________________________________________________________________
void BY21StrucFunc::KVectorFactors(const Interaction *interaction, double &kuv,
  ...
  // We include the BY21 K factors. The arxiv.org/pdf/2108.09240 publicatio also
  // accounts for a low energy transfer correction (KLW) The KLW is only
  // important in the SIS region (W<2GeV). As we scale it altogether with the
  // RES region, we do not need this factor. It also causes issues at low-W.
  // After discussing with A. Bodek, we agreed to comment this part out.
...
//____________________________________________________________________________
void BY21StrucFunc::KAxialFactors(const Interaction *interaction, double &kuv,
                                  double &kdv, double &kus, double &kds,
                                  double &kss) const {

  // https://arxiv.org/pdf/2108.09240 Sec 11.2
  // We apply a different K factor for axial contribtions, as given in Eq.
  // 49-50. There is a single correction for valance quarks, and a different
  // single correction for sea quarks.
  double myQ2 = this->Q2(interaction);
...
double BY21StrucFunc::H(const Interaction *interaction) const {
  ...
  // Overrides QPMDISStrucFuncBase::H() function to compute the correction of
  // the BY 2021 update The correction is given by Eq. 34 of
...
double BY21StrucFunc::KCharm(const Interaction *interaction, double Mf) const {
  // Overrides QPMDISStrucFuncBase::KCharm() function to compute the correction
  // of the BY 2021 update The correction corresponds to Sec 8 of
  // https://arxiv.org/pdf/2108.09240
...
double BY21StrucFunc::R(const Interaction *interaction) const {
  ...
  // Evaluate correction for Q2 < 0.3 GeV2/c4 according to Sec. 7 of
  // https://arxiv.org/pdf/2108.09240
  double Q2 = this->Q2(interaction);
  ...
  // The parameter, fRQ2min, is setup to 0.3 in the configuration. The default
  ...
  double Theta =
      1 + 12.0 * (Q2 / (Q2 + 1.)) * pow(0.125, 2) / (pow(0.125, 2) + x2);

  // R1998 is defined as the average of Ra, Rb and Rc, each parameterized to
  // accomodate new data at low x arXiv:hep-ex/9808028
  ...
  Ra +=
      a2 * (1 + a4 * x + a5 * x2) * pow(x, a6) / pow(Q8 + pow(a3, 4), 1. / 4.);
