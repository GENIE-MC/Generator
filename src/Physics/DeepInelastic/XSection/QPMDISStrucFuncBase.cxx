...
//____________________________________________________________________________
QPMDISStrucFuncBase::QPMDISStrucFuncBase() : DISStructureFuncModelI() {
  this->InitPDF();
}
//____________________________________________________________________________
...
void QPMDISStrucFuncBase::LoadConfig(void) {
  ...
  fDISNuclCorr =
      dynamic_cast<const DISNuclearModelI *>(this->SubAlg("DISNuclModel"));
...
void QPMDISStrucFuncBase::Calculate(const Interaction *interaction) const {
  ...
  if (tgt.HitQrkIsSet()) {
    ...
    if (is_nu && is_CC && is_c)
      return;
    if (is_nu && is_CC && is_dbar)
      return;
    if (is_nu && is_CC && is_sbar)
      return;
    if (is_nubar && is_CC && is_ubar)
      return;
    if (is_nubar && is_CC && is_cbar)
      return;
    if (is_nubar && is_CC && is_d)
      return;
    if (is_nubar && is_CC && is_s)
      return;
  }
  ...
  //
  // Compute K factors which effectively modify the coupling to the boson at low
  // Q2 The same K factors are used for neutron and proton hit nucleons The
  // modification enters differently the calculations of CC, NC F2 and F3.
  //
  ...
  this->KVectorFactors(interaction, kV_val_u, kV_val_d, kV_sea_u, kV_sea_d,
                       kV_sea_s);

#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DISSF", pDEBUG) << "K-Factors:";
  LOG("DISSF", pDEBUG) << "U: KV_Val = " << kV_val_u
                       << ", KV_Sea = " << kV_sea_u;
  LOG("DISSF", pDEBUG) << "D: KV_Val = " << kV_val_d
                       << ", KV_Sea = " << kV_sea_d;
  ...
  this->KAxialFactors(interaction, kA_val_u, kA_val_d, kA_sea_u, kA_sea_d,
                      kA_sea_s);

#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DISSF", pDEBUG) << "K-Factors:";
  LOG("DISSF", pDEBUG) << "U: KA_Val = " << kA_val_u
                       << ", KA_Sea = " << kA_sea_u;
  LOG("DISSF", pDEBUG) << "D: KA_Val = " << kA_val_d
                       << ", KA_Sea = " << kA_sea_d;
  ...
  if (is_NC || is_dmi) {
    ...
    if (!is_nu && !is_nubar && !is_dm)
      return;

    double GL =
        (is_nu) ? (0.5 - (2. / 3.) * fSin2thw) : (-(2. / 3.) * fSin2thw); // clu
    double GR =
        (is_nu) ? (-(2. / 3.) * fSin2thw) : (0.5 - (2. / 3.) * fSin2thw); // cru
    double GLp =
        (is_nu) ? (-0.5 + (1. / 3.) * fSin2thw) : ((1. / 3.) * fSin2thw); // cld
    double GRp =
        (is_nu) ? ((1. / 3.) * fSin2thw) : (-0.5 + (1. / 3.) * fSin2thw); // crd
    ...
    q2 +=
        (switch_us * fus + switch_c * fc) * (kV_sea_u * gvu2 + kA_sea_u * gau2);
  ...
  if (is_CC) {
    ...
    if (is_nu) {
      q = (switch_dv * fdv * (kV_val_d + kA_val_d) +
           switch_ds * fds * (kV_sea_d + kA_sea_d)) *
          fVud2;
      q += (switch_dv * fdv_c * (kV_val_d + kA_val_d) +
            switch_ds * fds_c * (kV_sea_d + kA_sea_d)) *
           fVcd2;
      ...
      q = (switch_uv * fuv * (kV_val_u + kA_val_u) +
           switch_us * fus * (kV_sea_u + kA_sea_u)) *
          fVud2;
      q += (switch_uv * fuv * (kV_val_u + kA_val_u) +
            switch_us * fus * (kV_sea_u + kA_sea_u)) *
           fVus2;
    ...
    if (is_nu) {
      ...
      q = (switch_uv * fuv * sqrt(kV_val_u * kA_val_u) +
           switch_us * fus * sqrt(kV_sea_u * kA_sea_u)) *
          fVud2;
      q += (switch_uv * fuv * sqrt(kV_val_u * kA_val_u) +
            switch_us * fus * sqrt(kV_sea_u * kA_sea_u)) *
           fVus2;
  ...
  if (is_EM) {
    ...
    if (!pdg::IsChargedLepton(probe_pdgc))
      return;
    ...
    double qu =
        sq23 * (switch_uv * fuv * kV_val_u + switch_us * fus * kV_sea_u);
    double qd =
        sq13 * (switch_dv * fdv * kV_val_d + switch_ds * fds * kV_sea_d);
  ...
  if (fUse2016Corrections) {
    ...
    // although it has not been explicitly asked to A.Bodek if it should be
    // done.
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DISSF", pDEBUG) << "F1-F5 = " << fF1 << ", " << fF2 << ", " << fF3
                       << ", " << fF4 << ", " << fF5;
#endif
}
//____________________________________________________________________________
...
//____________________________________________________________________________
double QPMDISStrucFuncBase::ScalingVar(const Interaction *interaction,
                                       double Mf) const {
  return interaction->Kine().x();
}
//____________________________________________________________________________
void QPMDISStrucFuncBase::KVectorFactors(const Interaction *, double &kuv,
                                         double &kdv, double &kus, double &kds,
                                         double &kss) const {
...
//____________________________________________________________________________
void QPMDISStrucFuncBase::KAxialFactors(const Interaction *, double &kuv,
                                        double &kdv, double &kus, double &kds,
                                        double &kss) const {
  // This is an abstract class: no model-specific correction
  // The PDF scaling variables are set to 1
...
void QPMDISStrucFuncBase::CalcPDFs(const Interaction *interaction) const {
  ...
  if (above_charm) {
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
    LOG("DISSF", pDEBUG) << "The event is above the charm threshold (mcharm = "
                         << fMc << ")";
#endif
    if (fCharmOff) {
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
        LOG("DISSF", pDEBUG) << "Calculating PDFs @ xc (slow rescaling) = " << x
                             << ", Q2 = " << Q2val;
#endif
    ...
    LOG("DISSF", pDEBUG) << "The event is below the charm threshold (mcharm = "
                         << fMc << ")";
  }
  ...
  double NuclCorr =
      fIncludeNuclMod ? fDISNuclCorr->DISACorrection(interaction) : 1;
...
double QPMDISStrucFuncBase::KCharm(const Interaction *interaction,
                                   double Mf) const {
...
double QPMDISStrucFuncBase::H(const Interaction *interaction) const {
  // The correction corresponds to Sec ??? eq ??? of
  // https://arxiv.org/pdf/2108.09240
  return 1;
}
