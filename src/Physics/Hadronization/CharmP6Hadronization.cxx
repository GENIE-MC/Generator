...
#include "Physics/Hadronization/CharmP6Hadronization.h"
...
//____________________________________________________________________________
CharmP6Hadronization::CharmP6Hadronization() :
EventRecordVisitorI("genie::CharmP6Hadronization")
{
  this->Initialize();
}
//____________________________________________________________________________
CharmP6Hadronization::CharmP6Hadronization(string config) :
EventRecordVisitorI("genie::CharmP6Hadronization", config)
{
  this->Initialize();
}
//____________________________________________________________________________
...
//____________________________________________________________________________
void CharmP6Hadronization::Initialize(void) const
{
#ifdef __GENIE_PYTHIA6_ENABLED__
...
//____________________________________________________________________________
void CharmP6Hadronization::ProcessEventRecord(GHepRecord * event) const
{
  Interaction * interaction = event->Summary();
  TClonesArray * particle_list = this->Hadronize(interaction);

  if(! particle_list ) {
    LOG("CharmP6Hadronization", pWARN) << "Got an empty particle list. Hadronizer failed!";
    LOG("CharmP6Hadronization", pWARN) << "Quitting the current event generation thread";
...
//____________________________________________________________________________
TClonesArray * CharmP6Hadronization::Hadronize(
...
//____________________________________________________________________________
int CharmP6Hadronization::GenerateCharmHadron(int nu_pdg, double EvLab) const
{
...
//____________________________________________________________________________
double CharmP6Hadronization::Weight(void) const 
{
  return 1. ;
}

//____________________________________________________________________________
void CharmP6Hadronization::Configure(const Registry & config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void CharmP6Hadronization::Configure(string config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void CharmP6Hadronization::LoadConfig(void)
{
  ...
  if ( ! utils::str::Convert(bits, ec) ) {
    LOG("CharmP6Hadronization", pFATAL) <<
    		"Failed to decode CharmFrac-E string: ";
    LOG("CharmP6Hadronization", pFATAL) << "string: "<< raw ;
  ...
  if ( d0frac.size() != ec.size() ) {
	  LOG("CharmP6Hadronization", pFATAL) << "E entries don't match D0 fraction entries";
	  LOG("CharmP6Hadronization", pFATAL) << "E:  " << ec.size() ;
	  LOG("CharmP6Hadronization", pFATAL) << "D0: " << d0frac.size() ;
	  invalid_configuration = true ;
  }
  ...
  // load D+ fractions
    ...
    if ( dpfrac.size() != ec.size() ) {
  	  LOG("CharmP6Hadronization", pFATAL) << "E entries don't match D+ fraction entries";
  	  LOG("CharmP6Hadronization", pFATAL) << "E:  " << ec.size() ;
  	  LOG("CharmP6Hadronization", pFATAL) << "D+: " << dpfrac.size() ;
  	  invalid_configuration = true ;
    }
    ...
    if ( dsfrac.size() != ec.size() ) {
    	LOG("CharmP6Hadronization", pFATAL) << "E entries don't match Ds fraction entries";
    	LOG("CharmP6Hadronization", pFATAL) << "E:  " << ec.size() ;
    	LOG("CharmP6Hadronization", pFATAL) << "Ds: " << dsfrac.size() ;
    	invalid_configuration = true ;
    }
