...
{
    LOG("DMELEvent", pINFO) << "Generating QE event kinematics...";
    ...
    // the next attempted event will be forced to DMEL again.
    ...
    while(1) {
        ...
        //         }
        //         else{LOG("DMELEvent",pDEBUG) << "ERROR - incoming particle not a proton or neutron" << std::endl;}
        //
        ...
        //
        //         //LOG("DMELEvent",pDEBUG) << "offshell energy = "<<EN_offshell << ", fRemovalEnergy = "<<w << std::endl;
        //         //LOG("DMELEvent",pDEBUG) << "binding energy = "<< Eb << std::endl;
        //
        ...
        //          LOG("DMELEvent", pINFO) << "Event below threshold, reject throw and try again!";
        //          continue;
        //        }
        ...
        //        LOG("DMELEvent", pNOTICE) << "dsigma/dQ2 (random) = " << t/(1E-38*units::cm2) << " 1E-38 cm^2/GeV^2";
        ...
        if(accept) {
            ...
            // get dark matter energy at struck nucleon rest frame and the
            ...
            LOG("DMELEvent",pNOTICE) << "pn: " << p4ptr.X() << ", " <<p4ptr.Y() << ", " <<p4ptr.Z() << ", " <<p4ptr.E();
            ...
            LOG("DMELEvent", pINFO) << "Reject current throw...";
        }
...
//____________________________________________________________________________
double DMELEventGenerator::ComputeXSec( Interaction * interaction, double costheta, double phi) const
{
    Target * tgt = interaction->InitState().TgtPtr();
    TLorentzVector * p4 = tgt->HitNucP4Ptr();
    ...
    //EN_offshell = Mn - w; 
    //LOG("DMELEvent",pDEBUG) << "Using FermiMoveBenharSF and defined energy" << std::endl;
    //}
    ...
    //LOG("DMELEvent",pDEBUG) << "Using FermiMoveDefault and defined energy" << std::endl;
    //}
    ...
    double outMomentum = TMath::Sqrt(outLeptonEnergy*outLeptonEnergy - lepMass*lepMass);
    //LOG("DMELEvent",pDEBUG) << "calculated root s and outLeptonEnergy" << std::endl;
    ...
    /*LOG("DMELEvent",pDEBUG) << "costheta = " << costheta << ", phi = " << phi << std::endl;
      LOG("DMELEvent",pDEBUG) << "lepton = " << utils::print::P4AsString(&lepton) << std::endl;
      LOG("DMELEvent",pDEBUG) << "outNucleon = " << utils::print::P4AsString(&outNucleon) << std::endl;
      LOG("DMELEvent",pDEBUG) << "inNucleon = " << utils::print::P4AsString(p4) << std::endl;
      LOG("DMELEvent",pDEBUG) << "s = " << s << ", outLepE = " << outLeptonEnergy << "outMom = " << outMomentum << std::endl;
      LOG("DMELEvent",pDEBUG) << "outMom^2 = " << outLeptonEnergy*outLeptonEnergy - lepMass*lepMass << std::endl;*/
    
    /*LOG("DMELEvent",pDEBUG) << "Lepton COM EvGen:\n";
      lepton.Print();
      LOG("DMELEvent",pDEBUG) << "outNucleon COM EvGen:\n";
      outNucleon.Print();*/
    ...
    //LOG("DMELEvent", pINFO) << "converted to lab frame";
    
    TLorentzVector leptonCOM = TLorentzVector(lepton);
    
    lepton.Boost(beta);
    outNucleon.Boost(beta);
    //LOG("DMELEvent", pINFO) << "lepton boosted = " << utils::print::P4AsString(&lepton),
    //LOG("DMELEvent", pINFO) << "outNucleon boosted = " << utils::print::P4AsString(&outNucleon);
    /*
      LOG("DMELEvent", pINFO) << "neutrino LAB EvGen: ";
      interaction->InitState().GetProbeP4(kRfLab)->Print();
      LOG("DMELEvent", pINFO) << "inNucleon LAB EvGen:\n";
      interaction->InitState().Tgt().HitNucP4().Print();
      LOG("DMELEvent", pINFO) << "Lepton LAB EvGen:\n";
      lepton.Print();
      LOG("DMELEvent", pINFO) << "outNucleon LAB EvGen:\n";
      outNucleon.Print();
      LOG("DMELEvent", pINFO) << "beta EvGen:";
      beta.Print();*/
    ...
    //LOG("DMELEvent", pINFO) << "min angle = " << fMinAngleEM;
...
//___________________________________________________________________________
TVector3 DMELEventGenerator::COMframe2Lab(InitialState initialState) const
{
...
//___________________________________________________________________________
double DMELEventGenerator::COMJacobian(TLorentzVector lepton, TLorentzVector leptonCOM, TLorentzVector outNucleon, TVector3 beta) const
{
