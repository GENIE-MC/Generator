...
#include "Physics/BoostedDarkMatter/EventGen/DMEKinematicsGenerator.h"
...
KineGeneratorWithCache("genie::DMEKinematicsGenerator")
{

}
//___________________________________________________________________________
DMEKinematicsGenerator::DMEKinematicsGenerator(string config) :
KineGeneratorWithCache("genie::DMEKinematicsGenerator", config)
{
...
//___________________________________________________________________________
void DMEKinematicsGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
  if(fGenerateUniformly) {
    LOG("DMEKinematics", pNOTICE)
  ...
  while(1) {
     ...
     LOG("DMEKinematics", pINFO) << "Trying: y = " << y;
     ...
     if(accept) {
        LOG("DMEKinematics", pINFO) << "Selected: y = " << y;
        ...
        // wght = (phase space volume)*(differential xsec)/(event total xsec)
         if(fGenerateUniformly) {
           ...
           LOG("DMEKinematics", pNOTICE)  << "Kinematics wght = "<< wght;

           // apply computed weight to the current event weight
           wght *= evrec->Weight();
           LOG("DMEKinematics", pNOTICE) << "Current event wght = " << wght;
...
//___________________________________________________________________________
double DMEKinematicsGenerator::ComputeMaxXSec(
...
//___________________________________________________________________________
double DMEKinematicsGenerator::Energy(const Interaction * interaction) const
{
