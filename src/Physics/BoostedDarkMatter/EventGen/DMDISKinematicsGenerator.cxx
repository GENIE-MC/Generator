...
#include "Physics/BoostedDarkMatter/EventGen/DMDISKinematicsGenerator.h"
...
KineGeneratorWithCache("genie::DMDISKinematicsGenerator")
{

}
//___________________________________________________________________________
DMDISKinematicsGenerator::DMDISKinematicsGenerator(string config) :
KineGeneratorWithCache("genie::DMDISKinematicsGenerator", config)
{
...
//___________________________________________________________________________
void DMDISKinematicsGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
  if(fGenerateUniformly) {
    LOG("DMDISKinematics", pNOTICE)
  ...
  if(W.max <=0 || W.min>=W.max) {
     LOG("DMDISKinematics", pWARN) << "No available phase space";
  ...
  LOG("DMDISKinematics", pNOTICE) << "x: [" << xl.min << ", " << xl.max << "]";
  LOG("DMDISKinematics", pNOTICE) << "y: [" << yl.min << ", " << yl.max << "]";
	...
	double J = 1;
     ...
     if(accept) {
         ...
         if(fGenerateUniformly) {
            ...
            LOG("DMDISKinematics", pNOTICE)  << "Kinematics wght = "<< wght;

            // apply computed weight to the current event weight
            wght *= evrec->Weight();
            LOG("DMDISKinematics", pNOTICE) << "Current event wght = " << wght;
...
//____________________________________________________________________________
double DMDISKinematicsGenerator::ComputeMaxXSec(
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DMDISKinematics", pDEBUG)<< "Computing max xsec in allowed phase space";
#endif
  double max_xsec = 0.0;
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
     LOG("DMDISKinematics", pDEBUG) << "y = " << gy;
#endif
     double xseclast_x = -1;
     bool increasing_x;
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  SLOG("DMDISKinematics", pDEBUG) << interaction->AsString();
  SLOG("DMDISKinematics", pDEBUG) << "Max xsec in phase space = " << max_xsec;
  SLOG("DMDISKinematics", pDEBUG) << "Computed using alg = " << *fXSecModel;
#endif
