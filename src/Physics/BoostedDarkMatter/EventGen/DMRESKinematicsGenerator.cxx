...
//assert(W.min>=0. && W.min<W.max);
  ...
  while(1) {
     ...
     if(fGenerateUniformly) {
       ...
       if(iter==1) {
         LOG("DMRESKinematics", pINFO) << "Initializing the sampling envelope";
         if(!fEnvelope) {
             LOG("DMRESKinematics", pFATAL) << "Null sampling envelope!";
             exit(1);
         }
     ...
     if(!fGenerateUniformly) {
        // > dark matter scattering (using importance sampling envelope)
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DMRESKinematics", pDEBUG) << "Scanning phase space for E= " << E;
#endif
  //bool scan1d = (E>1.0);
  bool scan1d = false;
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DMRESKinematics", pDEBUG) << interaction->AsString();
  LOG("DMRESKinematics", pDEBUG) << "Max xsec in phase space = " << max_xsec;
  LOG("DMRESKinematics", pDEBUG) << "Computed using " << fXSecModel->Id();
#endif
