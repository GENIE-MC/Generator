                 ...
                 const Interaction * interaction, KinePhaseSpace_t kps) const
{
  ...
  double A  = fgZp4 * TMath::Power(Ev,3) * me / (4.0 * kPi * (Ev2 - ml2) * TMath::Power(MZ2 + 2.0*Ev*me*y,2));
  ...
  if( proc_info.IsDarkMatter() && fVelMode == 0 ) 
  {
    ...
    double T1 = 1 + TMath::Power(1.0 - y,2); 
    double T2 = y * me / Ev;
    double T3 = y * ml2 / Ev / me;
    double T4 = 2.0 * ml2 / Ev2;
    double TL = 2.0*TMath::Power(2.0*y * me * ml / MZ2 + ml/Ev,2);
    ...
    if ( pdg::IsDarkMatter(inu) ) {
      xsec += 4.0 * (1.0 - TMath::Power(1.0-y,2))*QdmVA*QeVA;
    }
    else {
      xsec -= 4.0 * (1.0 - TMath::Power(1.0-y,2))*QdmVA*QeVA;
    }
