...
//____________________________________________________________________________
ReinSehgalDMRESXSecWithCacheFast::ReinSehgalDMRESXSecWithCacheFast() :
XSecIntegratorI()
{

}
//____________________________________________________________________________
ReinSehgalDMRESXSecWithCacheFast::ReinSehgalDMRESXSecWithCacheFast(string nm) :
XSecIntegratorI(nm)
{

}
//____________________________________________________________________________
ReinSehgalDMRESXSecWithCacheFast::ReinSehgalDMRESXSecWithCacheFast(string nm,string conf):
...
//____________________________________________________________________________
void ReinSehgalDMRESXSecWithCacheFast::CacheResExcitationXSec(
...
//  assert(fIntegrator);
  ...
  double ml    = in->InitState().Probe()->Mass();
  ...
  for(unsigned int ires = 0; ires < nres; ires++) {
         ...
         LOG("ReinSehgalDMResCF", pNOTICE) << "E threshold = " << Ethr;
         ...
         for(int ie=0; ie<nknots; ie++) {
             double xsec = 0.;
             double Ev   = E[ie];
	     double pv   = TMath::Sqrt(Ev*Ev - ml*ml);
             p4.SetPxPyPzE(0,0,pv,Ev);
               ...
               if(rW.max<rW.min || rQ2.max<rQ2.min || rW.min<0 || rQ2.min<0) {
                  LOG("ReinSehgalResCF", pINFO)
                              << "** Not allowed kinematically, xsec=0";
                                  ...
                                  ROOT::Math::IBaseFunctionMultiDim * func= new utils::gsl::d2XSecDMRESFast_dWQ2_E(fSingleResXSecModel, interaction);
...
//____________________________________________________________________________
string ReinSehgalDMRESXSecWithCacheFast::CacheBranchName(
...
// GSL wrappers
//____________________________________________________________________________
genie::utils::gsl::d2XSecDMRESFast_dWQ2_E::d2XSecDMRESFast_dWQ2_E(
     const XSecAlgorithmI * m, const Interaction * i) :
ROOT::Math::IBaseFunctionMultiDim(),
fModel(m),
fInteraction(i)
{
  LOG("ReinSehgalDMResCF", pNOTICE) << "Creating a new integration";
        kps = fInteraction->PhaseSpacePtr();
        Range1D_t Wl  = kps->WLim();
        fWmin = Wl.min;
        fWmax = Wl.max;
        Registry fConfig = (const_cast<XSecAlgorithmI *>(fModel))->GetConfig();
        LOG("ReinSehgalDMResCF", pNOTICE) << "W range: " << fWmin << " " << fWmax;
...
unsigned int genie::utils::gsl::d2XSecDMRESFast_dWQ2_E::NDim(void) const
{
  return 2;
}
double genie::utils::gsl::d2XSecDMRESFast_dWQ2_E::DoEval(const double * xin) const
{
// inputs:
//    W
//    Q2
...
//
  ...
  //  LOG("ReinSehgalDMResCF", pNOTICE) << "Q2 range: " << Q2l.min << " " << Q2l.max;
...
ROOT::Math::IBaseFunctionMultiDim *
   genie::utils::gsl::d2XSecDMRESFast_dWQ2_E::Clone() const
{
  return
    new genie::utils::gsl::d2XSecDMRESFast_dWQ2_E(fModel,fInteraction);
}
//____________________________________________________________________________
