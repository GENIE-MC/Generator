...
#ifndef _RADIATIVE_DECAYER_H_
#define _RADIATIVE_DECAYER_H_
...
class RadiativeDecayer : public DecayModelI {

public:
  RadiativeDecayer();
  RadiativeDecayer(string config);
  virtual ~RadiativeDecayer();

  // implement the DecayModelI interface
  bool           IsHandled      (int pdgc)                      const;
  void           Initialize     (void)                          const;
  TClonesArray * Decay          (const DecayerInputs_t & inp)   const;
  double 	 findEnergyLoss (const DecayerInputs_t & inp)	const;
