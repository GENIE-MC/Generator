...
#include "Physics/HadronTransport/HNIntranuke2014.h"
...
Intranuke2014("genie::HNIntranuke2014")
{

}
//___________________________________________________________________________
HNIntranuke2014::HNIntranuke2014(string config) :
Intranuke2014("genie::HNIntranuke2014",config)
{
...
//___________________________________________________________________________
void HNIntranuke2014::ProcessEventRecord(GHepRecord * evrec) const
{
  LOG("HNIntranuke2014", pNOTICE) 
  ...
  LOG("HNIntranuke2014", pINFO) << "Done with this event";
}
//___________________________________________________________________________
void HNIntranuke2014::SimulateHadronicFinalState(GHepRecord* ev, GHepParticle* p) const
{
// Simulate a hadron interaction for the input particle p in HN mode
//
  if(!p || !ev)
    {
      LOG("HNIntranuke2014", pERROR) << "** Null input!";
      return;
    }
  ...
  if(!(is_pion || is_baryon  || is_gamma))
    {
      LOG("HNIntranuke2014", pERROR) << "** Cannot handle particle: " << p->Name();
      return;
    }
  try
    {
	...
	{
	  LOG("HNIntranuke2014", pERROR) << "** Couldn't select a fate";
	  LOG("HNIntranuke2014", pERROR) << "** Num Protons: " << fRemnZ 
				     << ",  Num Neutrons: "<<(fRemnA-fRemnZ);
	  LOG("HNIntranuke2014", pERROR) << "** Particle: " << "\n" << (*p);
	  //LOG("HNIntranuke2014", pERROR) << "** Event Record: " << "\n" << (*ev);
  ...
  catch(exceptions::INukeException exception)
    {
      this->SimulateHadronicFinalState(ev,p);
       ...
       LOG("HNIntranuke2014", pNOTICE) << exception;
  ...
  while(iter++ < kRjMaxIterations) {
    ...
    if (pdgc==kPdgPiP || pdgc==kPdgPiM || pdgc==kPdgPi0) {
       ...
       double frac_cex      = this->FateWeight(pdgc, kIHNFtCEx)
	                            * fHadroData2014->Frac(pdgc, kIHNFtCEx,     ke, fRemnA, fRemnZ);
       ...
       double frac_abs      = this->FateWeight(pdgc, kIHNFtAbs)
	                            * fHadroData2014->Frac(pdgc, kIHNFtAbs,     ke, fRemnA, fRemnZ);
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
       LOG("HNIntranuke2014", pDEBUG) << "r = " << r << " (max = " << tf << ")";
#endif
...
//___________________________________________________________________________
double HNIntranuke2014::FateWeight(int pdgc, INukeFateHN_t fate) const
{
  ...
  if (np < 1 && nn < 1)
    {
      LOG("HNIntranuke2014", pERROR) << "** Nothing left in nucleus!!! **";
      return 0;
    }
  else
    {
    ...
    GHepRecord * ev, GHepParticle * p, INukeFateHN_t fate) const
{
  ...
  C3CM = fHadroData2014->IntBounce(p,t1code,scode,fate);
    if (C3CM<-1.) 
    {
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
	 ...
	 GHepRecord * ev, GHepParticle * p, INukeFateHN_t fate) const
{
  ...
  // get random scattering angle
  double C3CM = fHadroData2014->IntBounce(p,tcode,scode,fate);
  if (C3CM<-1.) 
    {
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
  ...
  bool pass = utils::intranuke2014::TwoBodyCollision(ev,pcode,tcode,scode,s2code,C3CM,
...
//___________________________________________________________________________
void HNIntranuke2014::InelasticHN(GHepRecord* ev, GHepParticle* p) const
{
  ...
  if (utils::intranuke2014::PionProduction(ev,p,s1,s2,s3,fRemnA,fRemnZ,fRemnP4,fDoFermi,fFermiFac,fFermiMomentum,fNuclmodel))
    {
...
//___________________________________________________________________________
void HNIntranuke2014::GammaInelasticHN(GHepRecord* ev, GHepParticle* p, INukeFateHN_t fate) const     
{
  ...
  if (rnd->RndFsi().Rndm() * (fHadroData2014 -> XSecGamp_fs() -> Evaluate(ke) +
			      fHadroData2014 -> XSecGamn_fs() -> Evaluate(ke)  )
      <= fHadroData2014 -> XSecGamp_fs() -> Evaluate(ke) ) { scode = kPdgProton;  }
  else                                                 { scode = kPdgNeutron; }

  //scode=fHadroData2014->AngleAndProduct(p,tcode,C3CM,fate);
...
//___________________________________________________________________________
int HNIntranuke2014::HandleCompoundNucleus(GHepRecord* ev, GHepParticle* p, int mom) const
{
  ...
  double rpreeq = rnd->RndFsi().Rndm();   // sdytman test
      ...
      if((p->KinE() < fEPreEq) )
	{
	  if(fRemnA>5&&rpreeq<0.12)
	      ...
	      utils::intranuke2014::PreEquilibrium(ev,sp,fRemnA,fRemnZ,fRemnP4,
...
//___________________________________________________________________________
bool HNIntranuke2014::HandleCompoundNucleusHN(GHepRecord* ev, GHepParticle* p) const
{
...
void HNIntranuke2014::LoadConfig(void)
{
  ...
  // report
  LOG("HNIntranuke2014", pINFO) << "Settings for Intranuke2014 mode: " << INukeMode::AsString(kIMdHN);
  LOG("HNIntranuke2014", pWARN) << "R0          = " << fR0 << " fermi";
  LOG("HNIntranuke2014", pWARN) << "NR          = " << fNR;
  LOG("HNIntranuke2014", pWARN) << "DelRPion    = " << fDelRPion;
  LOG("HNIntranuke2014", pWARN) << "DelRNucleon = " << fDelRNucleon;
  LOG("HNIntranuke2014", pWARN) << "HadStep     = " << fHadStep << " fermi";
  LOG("HNIntranuke2014", pWARN) << "NucAbsFac   = " << fNucAbsFac;
  LOG("HNIntranuke2014", pWARN) << "NucQEFac    = " << fNucQEFac;
  LOG("HNIntranuke2014", pWARN) << "NucCEXFac   = " << fNucCEXFac;
  LOG("HNIntranuke2014", pWARN) << "EPreEq      = " << fEPreEq;
  LOG("HNIntranuke2014", pWARN) << "FermiFac    = " << fFermiFac;
  LOG("HNIntranuke2014", pWARN) << "FreeStep    = " << fFreeStep;  // free step in fm
  LOG("HNIntranuke2014", pWARN) << "FermiMomtm  = " << fFermiMomentum;
  LOG("HNIntranuke2014", pWARN) << "DoFermi?    = " << ((fDoFermi)?(true):(false));
  LOG("HNIntranuke2014", pWARN) << "DoCmpndNuc? = " << ((fDoCompoundNucleus)?(true):(false));
}
//___________________________________________________________________________
