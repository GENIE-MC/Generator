...
//
  ...
  */
  ...
  ke = TMath::Max(INukeHadroData2015::fMinKinEnergy,   ke);
  ke = TMath::Min(INukeHadroData2015::fMaxKinEnergyHN, ke);
  ...
  if (is_pion and (INukeMode == "hN2015") and useOset and ke < 350.0)
    ...
    { sigtot = fHadroData2015 -> XSecPi0p_Tot() -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2015 -> XSecPi0n_Tot() -> Evaluate(ke)*(1-ppcnt);}
  else if (pdgc == kPdgPiM)
    { sigtot = fHadroData2015 -> XSecPipn_Tot() -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2015 -> XSecPipp_Tot() -> Evaluate(ke)*(1-ppcnt);}
  else if (pdgc == kPdgProton)
    {
      sigtot = fHadroData2015 -> XSecPp_Tot()   -> Evaluate(ke)*ppcnt;
      //sigtot+= fHadroData2015 -> XSecPn_Tot()   -> Evaluate(ke)*(1-ppcnt);
      ...
      if (Z*hc/137./x4.Vect().Mag() > E)  // Coulomb correction (Cohen, Concepts of Nuclear Physics, pg. 259-260)
	{
      ...
      if (INukeMode=="hN2015"){if(ke<E0){sigtot=0.0;}}  //empirical - needed to cut off large number of low energy nucleons
    }
  else if (pdgc == kPdgNeutron)
    { 
      sigtot = fHadroData2015 -> XSecPn_Tot()   -> Evaluate(ke)*ppcnt;
...
//____________________________________________________________________________
double genie::utils::intranuke2015::MeanFreePath_Delta(
...
//____________________________________________________________________________
double genie::utils::intranuke2015::ProbSurvival(
...
//  R0: R0 in R=R0*A^1/3 (units:fm). Def. 1.4
   ...
   while(1) {
     ...
     double mfp = 
       genie::utils::intranuke2015::MeanFreePath(pdgc,x4_curr,p4,A,Z,nRpi,nRnuc);
...
//____________________________________________________________________________
double genie::utils::intranuke2015::Dist2Exit(
...
//____________________________________________________________________________
double genie::utils::intranuke2015::Dist2ExitMFP(
   int pdgc, const TLorentzVector & x4, const TLorentzVector & p4, 
...
// 
   ...
   while(1) {
        ...
        double lambda = genie::utils::intranuke2015::MeanFreePath(pdgc,x4_curr,p4,A,Z);
        d_mfp += (step/lambda);
...
//____________________________________________________________________________
void genie::utils::intranuke2015::StepParticle(
...
//    and equilibrium
//    Alex Bell -- 6/17/2008
void genie::utils::intranuke2015::PreEquilibrium(
...
// Alex Bell -- 6/17/2008
void genie::utils::intranuke2015::Equilibrium(
  GHepRecord * ev, GHepParticle * p, int &RemnA, int &RemnZ, TLorentzVector &RemnP4,
  bool DoFermi, double FermiFac, const NuclearModelI* Nuclmodel, double NucRmvE, EINukeMode mode)
{
...
//___________________________________________________________________________
bool genie::utils::intranuke2015::TwoBodyCollision(
...
//___________________________________________________________________________
bool genie::utils::intranuke2015::TwoBodyKinematics(
...
//___________________________________________________________________________
bool genie::utils::intranuke2015::ThreeBodyKinematics(
  ...
  bool DoFermi, double FermiFac, double FermiMomentum, const NuclearModelI* Nuclmodel)
{
  ...
  // Aaron Meyer (7/15/10)
  //
  // Kinematics used in utils::intranuke2015::PionProduction
...
//___________________________________________________________________________
bool genie::utils::intranuke2015::PionProduction(
  ...
  //
   ...
   if ((p1code==kPdgPi0)||(p1code==kPdgPiP)||(p1code==kPdgPiM)) {
   ...
   } else if(p1code==kPdgProton||p1code==kPdgNeutron) //nucleon probes
    {
      ...
      double etapp2ppPi0 = 
	utils::intranuke2015::CalculateEta(pMass,tote,pMass,pMass+pMass,pLib->Find(111)->Mass());
      double etapp2pnPip = 
	utils::intranuke2015::CalculateEta(pLib->Find(p1code)->Mass(),tote,((p1code==kPdgProton)?pMass:nMass),
				       pMass+nMass,pLib->Find(211)->Mass());
      double etapn2nnPip = 
	utils::intranuke2015::CalculateEta(pMass,tote,nMass,nMass+nMass,pLib->Find(211)->Mass());
      double etapn2ppPim = 
	utils::intranuke2015::CalculateEta(pMass,tote,nMass,pMass+pMass,pLib->Find(211)->Mass());
   ...
   if(genie::utils::intranuke2015::ThreeBodyKinematics(
...
//___________________________________________________________________________
double genie::utils::intranuke2015::CalculateEta(double Minc, double nrg, double Mtarg,
			       double Mtwopart, double Mpi)
{
  ...
  //For pion cross sections, in utils::intranuke2015::PionProduction
...
//___________________________________________________________________________
bool genie::utils::intranuke2015::PhaseSpaceDecay(
...
double genie::utils::intranuke2015::sigmaTotalOset (
