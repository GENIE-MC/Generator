...
// rescattered by this cascade MC
  ...
  std::vector<int> Postion_evrec;
    ...
    while( (p = (GHepParticle *) piter.Next()) )
    {
     ...
     if(theType==G4INCL::UnknownParticle) { // INCL++ can't handle the particle
      sp->SetFirstMother(icurr); 
    sp->SetStatus(kIStStableFinalState);
    evrec->AddParticle(*sp);
    delete sp;
  ...
  Zexception = Z_i - Zresult;
   if(Zexception <=0 || Aexception<=0){ // Make sure that ARemn and Zremn >0 
    sp->SetFirstMother(icurr); 
    sp->SetStatus(kIStStableFinalState);
    evrec->AddParticle(*sp);
    delete sp;
    int nP(0);
    for (int it=0; it<ListeOfINCLresult.size();it++){
