...
const MAIDHelicityAmpl & MAIDHelicityAmplModelEMn::Compute( const Interaction * interaction ) const {
  ...
  switch(res) {
   ...
   case (kS11_1535) :
   {
     fAmpl.fA12 = -51.*(1.-4.75*q2)*exp(1.69*q2)/1000.;
     fAmpl.fS12  =  28.5*(1.-0.36*q2)*exp(1.55*q2)/1000.;
     break;
   }
   case (kD13_1520) :
   {
     fAmpl.fA12 = -77.*(1.+0.53*q2)*exp(1.55*q2)/1000.;
     fAmpl.fA32 = -154.*(1.-0.58*q2)*exp(1.75*q2)/1000.;
     fAmpl.fS12 =  13.6*(1.-15.7*q2)*exp(1.57*q2)/1000.;
     break;
   }
   case (kS11_1650) :
   {
     fAmpl.fA12 = 9.*(1.-0.13*q2)*exp(1.55*q2)/1000.;
     fAmpl.fA32 = 0. ;
     fAmpl.fS12 = 10.1*(1.+0.50*q2)*exp(1.55*q2)/1000.;
     break;
   }
   case (kD13_1700) :
   {
   ...
   case (kD15_1675) :
   {
     fAmpl.fA12 = -62.*(1.-0.01*q2)*exp(+2.00*q2)/1000.;
     fAmpl.fA32 = -84.*(1.-0.01*q2)*exp(+2.00*q2)/1000.;
     fAmpl.fS12 = 0.;
     break;
   }
   case (kS31_1620) :
   ...
   case (kP11_1440) :
   {
     fAmpl.fA12 = 54.1*(1.-0.95*q2)*exp(+1.77*q2)/1000.;
     fAmpl.fS12 = -41.5*(1.-2.98*q2)*exp(+1.55*q2)/1000.;
     fAmpl.fA32 = 0. ;
     break;
   }
   case (kP33_1600) :
   ...
   case (kP13_1720) :
   {
     fAmpl.fA12 = -3*(1.-12.7*q2)*exp(+1.55*q2)/1000.;
     fAmpl.fA32 = -31*(1.-4.99*q2)*exp(+1.55*q2)/1000.;
     fAmpl.fS12 = 0.;
     break;
   }
   case (kF15_1680) :
   {
     fAmpl.fA12 = 28*(1.-0.*q2)*exp(+1.2*q2)/1000.;
     fAmpl.fA32 = -38*(1.-4.09*q2)*exp(+1.75*q2)/1000.;
     fAmpl.fS12 = 0.;
     break;
   }
   case (kP31_1910) :
