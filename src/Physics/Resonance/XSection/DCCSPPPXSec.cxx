...
int DCCSPPPXSec::Wnode (double W) const
{
    ...
    // --- clamp ---
    if (iW < pos[0][1])     iW = 0;
    if (iW > pos[N-1][1]-1) iW = maxW - 2;
    
    // --- local correction ---
    while (iW > 0 && W < Wnodes[iW]) {
        --iW;
    }
    while (iW < pos[N-1][1]-1 && W >= Wnodes[iW + 1]) {
        ++iW;
    }
...
int DCCSPPPXSec::Q2node (double Q2) const
{
    ...
    // --- clamp ---
    if (iQ2 < pos[0][1])     iQ2 = 0;
    if (iQ2 > pos[N-1][1]-1) iQ2 = maxQ2 - 2;
    
    // --- local correction ---
    while (iQ2 > 0 && Q2 < Q2nodes[iQ2]) {
        --iQ2;
    }
    while (iQ2 < pos[N-1][1]-1 && Q2 >= Q2nodes[iQ2 + 1]) {
        ++iQ2;
    }
