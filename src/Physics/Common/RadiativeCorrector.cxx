...
void RadiativeCorrector::ProcessEventRecord(GHepRecord * evrec) const 
{
  ...
  while( (p = (GHepParticle *) piter.Next()) )
  {
	...
	if (vanderhagen) {
           ...
           double energyLossLimit = init_state_ptr->ProbeE(kRfLab) - fP4l.E(); 
	...
	if (simc) { 
	   ...
	   //lambda_e = (kAem/kPi)*( TMath::Log(4*pow(p->P4()->P(),2)/pow(kElectronMass,2)-1) + 2*TMath::Log(init_state_ptr->GetProbeP4(kRfLab)->P()/fP4l.E()) + TMath::Log(0.5*(1-kine->FSLeptonP4().CosTheta()) ) );
