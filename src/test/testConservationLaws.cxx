...
\program gtestConservationLaws

\brief   A simple test program to read-in a GHEP event tree and test whether
         the generated events obet basic conservation laws

         To run: gtestConservationLaws -f filename
...
int main(int argc, char ** argv)
{
  ...
  for(int i = 0; i< tree->GetEntries(); i++) {
    ...
    LOG("Main", pNOTICE) << "Checking event.... " << i;
    ...
    if(!ok) {
       if(!E_conserved) {
          LOG("Main", pERROR) << "** E is not conserved at this event";
       }
       if(!px_conserved) {
          LOG("Main", pERROR) << "** Px is not conserved at this event";
       }
       if(!py_conserved) {
          LOG("Main", pERROR) << "** Py is not conserved at this event";
       }
       if(!pz_conserved) {
          LOG("Main", pERROR) << "** Pz is not conserved at this event";
       }
       if(!Q_conserved) {
...
bool testCommandLineArgs(int argc)
{
  if(argc!=3) {
   LOG("Main", pERROR) 
       ...
       << "    Syntax: gtestConservationLaws -f root_filename";
   return false;
  }
  return true;
}
