...
#include "HadronTransport/HNIntranukeAlt.h"
...
//___________________________________________________________________________
//___________________________________________________________________________
HNIntranukeAlt::HNIntranukeAlt() :
IntranukeAlt("genie::HNIntranukeAlt")
{

}
//___________________________________________________________________________
HNIntranukeAlt::HNIntranukeAlt(string config) :
IntranukeAlt("genie::HNIntranukeAlt",config)
{
...
//___________________________________________________________________________
void HNIntranukeAlt::ProcessEventRecord(GHepRecord * evrec) const
{
  LOG("HNIntranukeAlt", pNOTICE) 
     << "************ Running HN Alternative MODE INTRANUKE ************";
     
  LOG("HNIntranukeAlt", pWARN) 
     << print::PrintFramedMesg(
         "Experimental code (INTRANUKE/hN model) - Run at your own risk");

  IntranukeAlt::ProcessEventRecord(evrec);

  LOG("HNIntranukeAlt", pINFO) << "Done with this event";
}
//___________________________________________________________________________
void HNIntranukeAlt::SimulateHadronicFinalState(GHepRecord* ev, GHepParticle* p) const
{
// Simulate a hadron interaction for the input particle p in HN mode
//
  if(!p || !ev)
    {
      LOG("HNIntranukeAlt", pERROR) << "** Null input!";
      return;
    }
  ...
  if(!(is_pion || is_baryon  || is_gamma))
    {
      LOG("HNIntranukeAlt", pERROR) << "** Cannot handle particle: " << p->Name();
      return;
    }
  try
    {
	...
	{
	  LOG("HNIntranukeAlt", pERROR) << "** Couldn't select a fate";
	  LOG("HNIntranukeAlt", pERROR) << "** Num Protons: " << fRemnZ 
				     << ",  Num Neutrons: "<<(fRemnA-fRemnZ);
	  LOG("HNIntranukeAlt", pERROR) << "** Particle: " << "\n" << (*p);
	  //LOG("HNIntranukeAlt", pERROR) << "** Event Record: " << "\n" << (*ev);
  ...
  catch(exceptions::INukeException exception)
    {
      this->SimulateHadronicFinalState(ev,p);
       ...
       LOG("HNIntranukeAlt", pNOTICE) << exception;

    }
}
//___________________________________________________________________________
INukeFateHN_t HNIntranukeAlt::HadronFateHN(const GHepParticle * p) const
{
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
       LOG("HNIntranukeAlt", pDEBUG) << "r = " << r << " (max = " << tf << ")";
#endif
...
//___________________________________________________________________________
double HNIntranukeAlt::FateWeight(int pdgc, INukeFateHN_t fate) const
{
  ...
  if (np < 1 && nn < 1)
    {
      LOG("HNIntranukeAlt", pERROR) << "** Nothing left in nucleus!!! **";
      return 0;
    }
  else
    {
    ...
    GHepRecord * ev, GHepParticle * p, INukeFateHN_t fate) const
{
  ...
  if(P3L < fFermiMomentum || P4L < fFermiMomentum)
    {
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
      LOG("HNIntranukeAlt",pINFO) << "AbsorbHN failed: Pauli blocking";
#endif
	 ...
	 GHepRecord * ev, GHepParticle * p, INukeFateHN_t fate) const
{
  ...
  {
    LOG("HNIntranukeAlt", pINFO) << "Elastic in hN failed calling TwoBodyCollision";
...
//___________________________________________________________________________
void HNIntranukeAlt::InelasticHN(GHepRecord* ev, GHepParticle* p) const
{
  ...
  if (utils::intranuke::PionProduction(ev,p,s1,s2,s3,fRemnA,fRemnZ,fRemnP4,fDoFermi,fFermiFac,fFermiMomentum,fNuclmodel))
    ...
    {
      LOG("HNIntranukeAlt", pNOTICE) << "Error: could not create pion production final state";
      exceptions::INukeException exception;
...
//___________________________________________________________________________
void HNIntranukeAlt::GammaInelasticHN(GHepRecord* ev, GHepParticle* p, INukeFateHN_t fate) const     
{
...
//___________________________________________________________________________
bool HNIntranukeAlt::HandleCompoundNucleus(GHepRecord* ev, GHepParticle* p, int mom) const
{
...
//___________________________________________________________________________
void HNIntranukeAlt::LoadConfig(void)
{
  ...
  // report
  LOG("HNIntranukeAlt", pINFO) << "Settings for IntranukeAlt mode: " << INukeMode::AsString(kIMdHN);
  LOG("HNIntranukeAlt", pWARN) << "R0          = " << fR0 << " fermi";
  LOG("HNIntranukeAlt", pWARN) << "NR          = " << fNR;
  LOG("HNIntranukeAlt", pWARN) << "DelRPion    = " << fDelRPion;
  LOG("HNIntranukeAlt", pWARN) << "DelRNucleon = " << fDelRNucleon;
  LOG("HNIntranukeAlt", pWARN) << "HadStep     = " << fHadStep << " fermi";
  LOG("HNIntranukeAlt", pWARN) << "NucAbsFac   = " << fNucAbsFac;
  LOG("HNIntranukeAlt", pWARN) << "NucQEFac    = " << fNucQEFac;
  LOG("HNIntranukeAlt", pWARN) << "NucCEXFac   = " << fNucCEXFac;
  LOG("HNIntranukeAlt", pWARN) << "EPreEq      = " << fEPreEq;
  LOG("HNIntranukeAlt", pWARN) << "FermiFac    = " << fFermiFac;
  LOG("HNIntranukeAlt", pWARN) << "FreeStep    = " << fFreeStep;  // free step in fm
  LOG("HNIntranukeAlt", pWARN) << "FermiMomtm  = " << fFermiMomentum;
  LOG("HNIntranukeAlt", pWARN) << "DoFermi?    = " << ((fDoFermi)?(true):(false));
  LOG("HNIntranukeAlt", pWARN) << "DoCmpndNuc? = " << ((fDoCompoundNucleus)?(true):(false));
}
//___________________________________________________________________________
