...
//___________________________________________________________________________
Intranuke2014::Intranuke2014(string name, string config) :
...
//___________________________________________________________________________
void Intranuke2014::ProcessEventRecord(GHepRecord * evrec) const
{
  // Do not continue if there is no nuclear target
  GHepParticle * nucltgt = evrec->TargetNucleus();
  if (!nucltgt) {
    LOG("HNIntranuke2014", pINFO) << "No nuclear target found - INTRANUKE exits";
    return;
  }
...
//___________________________________________________________________________
void Intranuke2014::GenerateVertex(GHepRecord * evrec) const
{
...
//___________________________________________________________________________
void Intranuke2014::TransportHadrons(GHepRecord * evrec) const
{
// transport all hadrons outside the nucleus
  ...
  while( (p = (GHepParticle *) piter.Next()) )
  {
    ...
    while ( this-> IsInNucleus(sp) ) 
    {
      ...
      utils::intranuke2014::StepParticle(sp, fHadStep);
    ...
    // Current snapshot
    //LOG("Intranuke2014", pINFO) << "Current event record snapshot: " << *evrec;
...
//___________________________________________________________________________
double Intranuke2014::GenerateStep(GHepRecord* /*evrec*/, GHepParticle* p) const
{
...
// from an exp(-d/L) distribution
  ...
  double L = utils::intranuke2014::MeanFreePath(p->Pdg(), *p->X4(), *p->P4(), fRemnA,
     fRemnZ, fDelRPion, fDelRNucleon);
  double d = -1.*L * TMath::Log(rnd->RndFsi().Rndm());

  LOG("Intranuke2014", pDEBUG)
            << "Mean free path = " << L << " fm / "
