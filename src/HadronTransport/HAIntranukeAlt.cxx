...
#include "HadronTransport/IntranukeAlt.h"
#include "HadronTransport/HAIntranukeAlt.h"
...
//___________________________________________________________________________
//___________________________________________________________________________
HAIntranukeAlt::HAIntranukeAlt() :
IntranukeAlt("genie::HAIntranukeAlt")
{

}
//___________________________________________________________________________
HAIntranukeAlt::HAIntranukeAlt(string config) :
IntranukeAlt("genie::HAIntranukeAlt",config)
{
...
//___________________________________________________________________________
void HAIntranukeAlt::ProcessEventRecord(GHepRecord * evrec) const
{
  LOG("HAIntranukeAlt", pNOTICE) 
     << "************ Running HA Alternative MODE INTRANUKE ************";

  IntranukeAlt::ProcessEventRecord(evrec);

  LOG("HAIntranukeAlt", pINFO) << "Done with this event";
}
//___________________________________________________________________________
void HAIntranukeAlt::SimulateHadronicFinalState(
...
//
  // check inputs
  if(!p || !ev) {
     LOG("HAIntranukeAlt", pERROR) << "** Null input!";
     return;
  }
  ...
  if(!is_handled) {
     LOG("HAIntranukeAlt", pERROR) << "** Can not handle particle: " << p->Name();
     return;     
  }
  ...
  if(fate == kIHAFtUndefined) {
     LOG("HAIntranukeAlt", pERROR) << "** Couldn't select a fate";
...
//___________________________________________________________________________
void HAIntranukeAlt::SimulateHadronicFinalStateKinematics(
  GHepRecord* ev, GHepParticle* p) const
{
  // get stored fate
...
//___________________________________________________________________________
INukeFateHA_t HAIntranukeAlt::HadronFateHA(const GHepParticle * p) const
{
  ...
  while(iter++ < kRjMaxIterations) {
    ...
    if (pdgc==kPdgPiP || pdgc==kPdgPiM || pdgc==kPdgPi0) {
	  ...
	  << "\n frac{" << INukeHadroFates::AsString(kIHAFtAbs)     << "} = " << frac_abs
       ...
       LOG("HAIntranukeAlt", pDEBUG) << "r = " << r << " (max = " << tf << ")";
#endif
...
//___________________________________________________________________________
double HAIntranukeAlt::PiBounce(void) const
{
...
//___________________________________________________________________________
double HAIntranukeAlt::PnBounce(void) const
{
         ...
         GHepRecord* ev, GHepParticle* p, INukeFateHA_t fate) const
{
  ...
  if(fRemnA<0 || fRemnZ<0) // best to stop it here and not try again.
    {
      LOG("HAIntranukeAlt", pWARN) << "Invalid Nucleus! : (A,Z) = ("<<fRemnA<<','<<fRemnZ<<')';
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      return;
    }
  ...
  if (!utils::intranuke::TwoBodyKinematics(Mp,Mt,t4PpL,t4PtL,t4P3L,t4P4L,C3CM,fRemnP4))
    {
      LOG("HAIntranukeAlt", pNOTICE) << "ElasHA() failed";
...
//___________________________________________________________________________
void HAIntranukeAlt::InelasticHA(
	GHepRecord* ev, GHepParticle* p, INukeFateHA_t fate) const
{
...
#endif
  if(ev->Probe() ) {
    LOG("HAIntranukeAlt", pINFO) << " probe KE = " << ev->Probe()->KinE();
  }
  ...
  if(fate==kIHAFtCEx)
	...
	{ LOG("HAIntranukeAlt", pWARN) << "InelasticHA() cannot handle fate: " 
  ...
  if ( fRemnA < 1 )    //we've blown nucleus apart, no need to retry anything - exit
    {
      LOG("HAIntranukeAlt",pNOTICE) << "InelasticHA() stops : not enough nucleons";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      return;
    }
    ...
    {
      LOG("HAIntranukeAlt",pWARN) << "InelasticHA() failed : too few protons in nucleus";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
  ...
  if (C3CM<-1.)   // hope this doesn't occur too often - unphysical but we just pass it on
    {
      LOG("HAIntranukeAlt", pWARN) << "unphysical angle chosen in InelasicHA - put particle outside nucleus";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      delete t;
      return;
  ...
  if(success)
    {
      ...
      double E3L = cl1->KinE();
      double E4L = cl2->KinE();
      LOG ("HAIntranukeAlt",pINFO) << "Successful quasielastic scattering or charge exchange";
      LOG("HAIntranukeAlt",pINFO)
	<< "C3CM = " << C3CM << "\n  P3 = " 
	<< P3L << "   " << E3L << "             P4 = " ;
  if(ev->Probe() ) { LOG("HAIntranukeAlt",pINFO)
  ...
  if (ev->Probe() && (E3L>ev->Probe()->KinE()||E4L>ev->Probe()->KinE()))  //is this redundant?
	{
      ...
      LOG("HAIntranukeAlt", pDEBUG) << "Nucleus : (A,Z) = ("<<fRemnA<<','<<fRemnZ<<')';
    } else
    {
      exceptions::INukeException exception;
          ...
          GHepRecord* ev, GHepParticle* p, INukeFateHA_t fate) const
{
  ...
  if (fate == kIHAFtPiProd) {
      ...
      if (success){
	LOG ("HAIntranukeAlt",pINFO) << " successful pion production fate";
	...
	LOG("HAIntranukeAlt", pNOTICE) << "Error: could not create pion production final state";
	exceptions::INukeException exception;
...
//   use same for kaons as there is no guidance
    {
      ...
      if (fRemnA<2)
      {
	  LOG("HAIntranukeAlt", pNOTICE) << "stop  propagation - could not create absorption final state: too few particles";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      return;
      }
      ...
      {
	LOG("HAIntranukeAlt", pNOTICE) << "stop propagation - could not create absorption final state: Pi- or K- cannot be absorbed by only neutrons";
	p->SetStatus(kIStStableFinalState);
	ev->AddParticle(*p);
	return;
      }
      ...
      {
	LOG("HAIntranukeAlt", pINFO) << "stop propagation - could not create absorption final state: Pi+ or K+ cannot be absorbed by only protons";
	p->SetStatus(kIStStableFinalState);
	ev->AddParticle(*p);
	return;
      }
	...
	{  // pi d -> N N, probability determined empirically with McKeown data
	  ...
	  LOG("HAIntranukeAlt",pINFO) << "choose 2 body absorption, probe, fs = " << pdgc <<"  "<< scode <<"  "<<s2code;
	  ...
	  if (C3CM<-1.) 
	    {
	      LOG("HAIntranukeAlt", pWARN) << "Inelastic() failed: IntBounce returned bad angle - try again";
	      exceptions::INukeException exception;
	  ...
	  if (utils::intranuke::TwoBodyKinematics(M3,M4,t4P1L,t4P2L,t4P3L,t4P4L,C3CM,fRemnP4,bindE))
	    ...
	    {
	      LOG("HAIntranukeAlt", pNOTICE) << "Inelastic in hA failed calling TwoBodyKineamtics";
	      exceptions::INukeException exception;
...
//add check to be certain
	  ...
	  //gam_ns = 10.;
	  LOG("HAIntranukeAlt", pINFO) << "nucleon absorption";
	  LOG("HAIntranukeAlt", pINFO) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	  //	  LOG("HAIntranukeAlt", pINFO) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	  LOG("HAIntranukeAlt", pINFO) << "--> gam_ns = " << gam_ns;
	}
      else if ( pdgc==kPdgPiP || pdgc==kPdgPi0 || pdgc==kPdgPiM) //pion probe
	{
	  ...
	  LOG("HAIntranukeAlt", pINFO) << "pion absorption";
	  LOG("HAIntranukeAlt", pINFO) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	  LOG("HAIntranukeAlt", pINFO) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	}
      else if (pdgc==kPdgKP || pdgc==kPdgKM) // kaon probe
	{
	  ...
	  Sig_ns = 0.1;
	  Sig_nd = 0.1;
	  LOG("HAIntranukeAlt", pINFO) << "kaon absorption - set ns, nd later";
	  //	  LOG("HAIntranukeAlt", pINFO) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	  //	  LOG("HAIntranukeAlt", pINFO) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	}
      else
	{
	  LOG("HAIntranukeAlt", pWARN) << "Inelastic() cannot handle absorption reaction for " << p->Name();
	}
      ...
      while (not_done)
	{
	  // infinite loop check
	  if (iter>=100) {
	    LOG("HAIntranukeAlt", pNOTICE) << "Error: could not choose absorption final state";
	    LOG("HAIntranukeAlt", pNOTICE) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	    LOG("HAIntranukeAlt", pNOTICE) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	    LOG("HAIntranukeAlt", pNOTICE) << "--> gam_ns = " << gam_ns;
	    LOG("HAIntranukeAlt", pNOTICE) << "--> A = " << fRemnA << ", Z = " << fRemnZ << ", Energy = " << ke;
	    exceptions::INukeException exception;
	  ...
	  if ( pdg::IsKaon (pdgc) ) //charged kaon probe - either 2 or 3 nucleons to stay simple
	      ...
	      while (not_found)
		{
		  ...
		  if (iter2>=100)
		    {
		      LOG("HAIntranukeAlt", pNOTICE) << "Error: stuck in random variable loop for ns";
		      LOG("HAIntranukeAlt", pNOTICE) << "--> mean of sum parent distr = " << ns0 << ", Stand dev = " << Sig_ns;
		      LOG("HAIntranukeAlt", pNOTICE) << "--> A = " << fRemnA << ", Z = " << fRemnZ << ", Energy = " << ke;
	  ...
	  LOG("HAIntranukeAlt", pINFO) << "ns = "<<ns<<", nd = "<<nd<<", np = "<<np<<", nn = "<<nn;
	  //LOG("HAIntranukeAlt", pNOTICE) << "RemA = "<<fRemnA<<", RemZ = "<<fRemnZ<<", probe = "<<pdgc;
	  ...
	  //check for problems befor executing phase space 'decay'
	    ...
	    LOG("HAIntranukeAlt",pINFO) << "success, iter = " << iter << "  np, nn = " << np << "  " << nn; 
	    ...
	    LOG("HAIntranukeAlt", pNOTICE) << "Final state chosen; # protons : " 
					<< np << ", # neutrons : " << nn;
	  }
	} //while(not_done)
      ...
      if ((np+nn)>18)
	{
	  ...
	  for (int i=0;i<5;i++)
	    {
	      LOG("HAIntranukeAlt", pINFO) << "List" << i << " size: " << listar[i]->size();
	      if (listar[i]->size() <2)
		{
		  exceptions::INukeException exception;
	  ...
	  if(success1 && success2 && success3 && success4 && success5)
	    {
	      LOG("HAIntranukeAlt", pINFO)<<"Successful many-body absorption - n>=18";
	    }
	  else 
	    {
	      // try to recover 
	      LOG("HAIntranukeAlt", pWARN) << "PhaseSpace decay fails for HadrCluster- recovery likely incorrect - rethrow event";
	      p->SetStatus(kIStStableFinalState);
	      ev->AddParticle(*p);
	      fRemnA+=np+nn;
	      fRemnZ+=np;
      ...
      else // less than 18 particles pion
	{
	  ...
	  LOG("HAIntranukeAlt", pINFO) << " list size: " << np+nn;
	  if (np+nn <2)
	    {
	      exceptions::INukeException exception;
	      exception.SetReason("too few particles for Phase Space decay - try again");
...
//___________________________________________________________________________
bool HAIntranukeAlt::HandleCompoundNucleus(
...
//___________________________________________________________________________
void HAIntranukeAlt::LoadConfig(void)
{
  ...
  // report
  LOG("HAIntranukeAlt", pINFO) << "Settings for INTRANUKE mode: " << INukeMode::AsString(kIMdHA);
  LOG("HAIntranukeAlt", pINFO) << "R0          = " << fR0 << " fermi";
  LOG("HAIntranukeAlt", pINFO) << "NR          = " << fNR;
  LOG("HAIntranukeAlt", pINFO) << "DelRPion    = " << fDelRPion;
  LOG("HAIntranukeAlt", pINFO) << "DelRNucleon = " << fDelRNucleon;
  LOG("HAIntranukeAlt", pINFO) << "HadStep     = " << fHadStep << " fermi";
  LOG("HAIntranukeAlt", pINFO) << "EPreEq      = " << fHadStep << " fermi";
  LOG("HAIntranukeAlt", pINFO) << "NucAbsFac   = " << fNucAbsFac;
  LOG("HAIntranukeAlt", pINFO) << "NucCEXFac   = " << fNucCEXFac;
  LOG("HAIntranukeAlt", pINFO) << "FermiFac    = " << fFermiFac;
  LOG("HAIntranukeAlt", pINFO) << "FreeStep    = " << fFreeStep;  // free step in fm
  LOG("HAIntranukeAlt", pINFO) << "FermiMomtm  = " << fFermiMomentum;
  LOG("HAIntranukeAlt", pINFO) << "DoFermi?    = " << ((fDoFermi)?(true):(false));
  LOG("HAIntranukeAlt", pINFO) << "DoCmpndNuc? = " << ((fDoCompoundNucleus)?(true):(false));
}
//___________________________________________________________________________
