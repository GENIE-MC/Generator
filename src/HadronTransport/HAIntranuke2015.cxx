...
#include "HadronTransport/HAIntranuke2015.h"
...
Intranuke2015("genie::HAIntranuke2015")
{

}
//___________________________________________________________________________
HAIntranuke2015::HAIntranuke2015(string config) :
Intranuke2015("genie::HAIntranuke2015",config)
{
...
//___________________________________________________________________________
void HAIntranuke2015::ProcessEventRecord(GHepRecord * evrec) const
{
  ...
  LOG("HAIntranuke2015", pINFO) << "Done with this event";
}
//___________________________________________________________________________
void HAIntranuke2015::SimulateHadronicFinalState(
...
//
  // check inputs
  if(!p || !ev) {
     LOG("HAIntranuke2015", pERROR) << "** Null input!";
     return;
  }
  ...
  if(!is_handled) {
     LOG("HAIntranuke2015", pERROR) << "** Can not handle particle: " << p->Name();
     return;     
  }
  ...
  if(fate == kIHAFtUndefined) {
     LOG("HAIntranuke2015", pERROR) << "** Couldn't select a fate";
...
//___________________________________________________________________________
void HAIntranuke2015::SimulateHadronicFinalStateKinematics(
  GHepRecord* ev, GHepParticle* p) const
{
  // get stored fate
...
//
  ...
  if (isPion and fUseOset and ke < 350.0) return HadronFateOset (); //edit
   ...
   if (pdgc==kPdgPiP || pdgc==kPdgPiM || pdgc==kPdgPi0) {
     ...
     LOG("HAIntranuke2015", pDEBUG) 
	  ...
	  << "\n frac{" << INukeHadroFates::AsString(kIHAFtAbs)     << "} = " << frac_abs
       ...
       LOG("HAIntranuke2015", pDEBUG) << "r = " << r << " (max = " << tf << ")";
#endif
         ...
         GHepRecord* ev, GHepParticle* p, INukeFateHA_t fate) const
{
  ...
  if(fRemnA<0 || fRemnZ<0) // best to stop it here and not try again.
    {
      LOG("HAIntranuke2015", pWARN) << "Invalid Nucleus! : (A,Z) = ("<<fRemnA<<','<<fRemnZ<<')';
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      return;
    }
  ...
  if (!utils::intranuke2015::TwoBodyKinematics(Mp,Mt,t4PpL,t4PtL,t4P3L,t4P4L,C3CM,fRemnP4))
    {
      LOG("HAIntranuke2015", pNOTICE) << "ElasHA() failed";
...
//___________________________________________________________________________
void HAIntranuke2015::InelasticHA(
	GHepRecord* ev, GHepParticle* p, INukeFateHA_t fate) const
{
...
#endif
  if(ev->Probe() ) {
    LOG("HAIntranuke2015", pINFO) << " probe KE = " << ev->Probe()->KinE();
  }
  ...
  if(fate==kIHAFtCEx)
	...
	{ LOG("HAIntranuke2015", pWARN) << "InelasticHA() cannot handle fate: " 
  ...
  if ( fRemnA < 1 )    //we've blown nucleus apart, no need to retry anything - exit
    {
      LOG("HAIntranuke2015",pNOTICE) << "InelasticHA() stops : not enough nucleons";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      return;
    }
    ...
    {
      LOG("HAIntranuke2015",pWARN) << "InelasticHA() failed : too few protons in nucleus";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
  ...
  double C3CM = fHadroData2015->IntBounce(cl,tcode,scode,h_fate);
  delete cl;
  if (C3CM<-1.)   // hope this doesn't occur too often - unphysical but we just pass it on
    {
      LOG("HAIntranuke2015", pWARN) << "unphysical angle chosen in InelasicHA - put particle outside nucleus";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      return;
    }  
  ...
  GHepParticle cl1(*p);
  GHepParticle cl2(t);
  bool success = utils::intranuke2015::TwoBodyCollision(ev,pcode,tcode,scode,s2code,C3CM,
  ...
  if(success)
    {
      ...
      double E3L = cl1.KinE();
      double E4L = cl2.KinE();
      LOG ("HAIntranuke2015",pINFO) << "Successful quasielastic scattering or charge exchange";
...
//___________________________________________________________________________
void HAIntranuke2015::Inelastic(
          GHepRecord* ev, GHepParticle* p, INukeFateHA_t fate) const
{
  ...
  if (fate == kIHAFtPiProd) {
      ...
      bool success = utils::intranuke2015::PionProduction(
         ev,p,&s1,&s2,&s3,fRemnA,fRemnZ,fRemnP4, fDoFermi,fFermiFac,fFermiMomentum,fNuclmodel);

      if (success){
	LOG ("HAIntranuke2015",pINFO) << " successful pion production fate";
	...
	LOG("HAIntranuke2015", pNOTICE) << "Error: could not create pion production final state";
	exceptions::INukeException exception;
...
//   use same for kaons as there is no guidance
    {
      ...
      if (fRemnA<2)
      {
	  LOG("HAIntranuke2015", pNOTICE) << "stop  propagation - could not create absorption final state: too few particles";
      p->SetStatus(kIStStableFinalState);
      ev->AddParticle(*p);
      return;
      }
      ...
      {
	LOG("HAIntranuke2015", pNOTICE) << "stop propagation - could not create absorption final state: Pi- or K- cannot be absorbed by only neutrons";
	p->SetStatus(kIStStableFinalState);
	ev->AddParticle(*p);
	return;
      }
      ...
      {
	LOG("HAIntranuke2015", pINFO) << "stop propagation - could not create absorption final state: Pi+ or K+ cannot be absorbed by only protons";
	p->SetStatus(kIStStableFinalState);
	ev->AddParticle(*p);
	return;
      }
	...
	{  // pi d -> N N, probability determined empirically with McKeown data
	  ...
	  LOG("HAIntranuke2015",pINFO) << "choose 2 body absorption, probe, fs = " << pdgc <<"  "<< scode <<"  "<<s2code;
	  ...
	  double C3CM = fHadroData2015->IntBounce(p,t1code,scode,fate_hN);
	  if (C3CM<-1.) 
	    {
	      LOG("HAIntranuke2015", pWARN) << "Inelastic() failed: IntBounce returned bad angle - try again";
	      exceptions::INukeException exception;
	  ...
	  //double bindE=0.0; 
	  if (utils::intranuke2015::TwoBodyKinematics(M3,M4,t4P1L,t4P2L,t4P3L,t4P4L,C3CM,fRemnP4,bindE))
	    {
	    ...
	    {
	      LOG("HAIntranuke2015", pNOTICE) << "Inelastic in hA failed calling TwoBodyKineamtics";
	      exceptions::INukeException exception;
...
//add check to be certain
	  ...
	  //gam_ns = 10.;
	  LOG("HAIntranuke2015", pINFO) << "nucleon absorption";
	  LOG("HAIntranuke2015", pINFO) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	  //	  LOG("HAIntranuke2015", pINFO) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	  LOG("HAIntranuke2015", pINFO) << "--> gam_ns = " << gam_ns;
	}
      else if ( pdgc==kPdgPiP || pdgc==kPdgPi0 || pdgc==kPdgPiM) //pion probe
	{
	  ...
	  LOG("HAIntranuke2015", pINFO) << "pion absorption";
	  LOG("HAIntranuke2015", pINFO) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	  LOG("HAIntranuke2015", pINFO) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	}
      else if (pdgc==kPdgKP || pdgc==kPdgKM) // kaon probe
	{
	  ...
	  Sig_ns = 0.1;
	  Sig_nd = 0.1;
	  LOG("HAIntranuke2015", pINFO) << "kaon absorption - set ns, nd later";
	  //	  LOG("HAIntranuke2015", pINFO) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	  //	  LOG("HAIntranuke2015", pINFO) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	}
      else
	{
	  LOG("HAIntranuke2015", pWARN) << "Inelastic() cannot handle absorption reaction for " << p->Name();
	}
      ...
      while (not_done)
	{
	  // infinite loop check
	  if (iter>=100) {
	    LOG("HAIntranuke2015", pNOTICE) << "Error: could not choose absorption final state";
	    LOG("HAIntranuke2015", pNOTICE) << "--> mean diff distr = " << nd0 << ", stand dev = " << Sig_nd;
	    LOG("HAIntranuke2015", pNOTICE) << "--> mean sum distr = " << ns0 << ", Stand dev = " << Sig_ns;
	    LOG("HAIntranuke2015", pNOTICE) << "--> gam_ns = " << gam_ns;
	    LOG("HAIntranuke2015", pNOTICE) << "--> A = " << fRemnA << ", Z = " << fRemnZ << ", Energy = " << ke;
	    exceptions::INukeException exception;
	  ...
	  if ( pdg::IsKaon (pdgc) ) //charged kaon probe - either 2 or 3 nucleons to stay simple
	      ...
	      while (not_found)
		{
		  ...
		  if (iter2>=100)
		    {
		      LOG("HAIntranuke2015", pNOTICE) << "Error: stuck in random variable loop for ns";
		      LOG("HAIntranuke2015", pNOTICE) << "--> mean of sum parent distr = " << ns0 << ", Stand dev = " << Sig_ns;
		      LOG("HAIntranuke2015", pNOTICE) << "--> A = " << fRemnA << ", Z = " << fRemnZ << ", Energy = " << ke;
	  ...
	  LOG("HAIntranuke2015", pINFO) << "ns = "<<ns<<", nd = "<<nd<<", np = "<<np<<", nn = "<<nn;
	  //LOG("HAIntranuke2015", pNOTICE) << "RemA = "<<fRemnA<<", RemZ = "<<fRemnZ<<", probe = "<<pdgc;
	  ...
	  //check for problems befor executing phase space 'decay'
	    ...
	    LOG("HAIntranuke2015",pINFO) << "success, iter = " << iter << "  np, nn = " << np << "  " << nn; 
	    ...
	    LOG("HAIntranuke2015", pNOTICE) << "Final state chosen; # protons : " 
					<< np << ", # neutrons : " << nn;
	  }
	} //while(not_done)
      ...
      if ((np+nn)>18)
	{
	  ...
	  for (int i=0;i<5;i++)
	    {
	      LOG("HAIntranuke2015", pINFO) << "List" << i << " size: " << listar[i]->size();
	      if (listar[i]->size() <2)
		{
		  exceptions::INukeException exception;
	  ...
	  bool success1 = utils::intranuke2015::PhaseSpaceDecay(ev,p0,*listar[0],fRemnP4,fNucRmvE,kIMdHA);
	  bool success2 = utils::intranuke2015::PhaseSpaceDecay(ev,p1,*listar[1],fRemnP4,fNucRmvE,kIMdHA);
	  bool success3 = utils::intranuke2015::PhaseSpaceDecay(ev,p2,*listar[2],fRemnP4,fNucRmvE,kIMdHA);
	  bool success4 = utils::intranuke2015::PhaseSpaceDecay(ev,p3,*listar[3],fRemnP4,fNucRmvE,kIMdHA);
	  bool success5 = utils::intranuke2015::PhaseSpaceDecay(ev,p4,*listar[4],fRemnP4,fNucRmvE,kIMdHA);
	  if(success1 && success2 && success3 && success4 && success5)
	    ...
	    {
	      // try to recover 
	      LOG("HAIntranuke2015", pWARN) << "PhaseSpace decay fails for HadrCluster- recovery likely incorrect - rethrow event";
	      p->SetStatus(kIStStableFinalState);
	      ev->AddParticle(*p);
	      fRemnA+=np+nn;
	      fRemnZ+=np;
      ...
      else // less than 18 particles pion
	{
	  ...
	  LOG("HAIntranuke2015", pINFO) << " list size: " << np+nn;
	  if (np+nn <2)
	    {
	      exceptions::INukeException exception;
	      exception.SetReason("too few particles for Phase Space decay - try again");
	  ...
	  bool success = utils::intranuke2015::PhaseSpaceDecay(ev,p,list,fRemnP4,fNucRmvE,kIMdHA);
	  if (success)
	    {
	      LOG ("HAIntranuke2015",pINFO) << "Successful many-body absorption, n<=18";
	    }
...
void HAIntranuke2015::LoadConfig(void)
{
  ...
  LOG("HAIntranuke2015", pINFO) << "R0          = " << fR0 << " fermi";
  LOG("HAIntranuke2015", pINFO) << "NR          = " << fNR;
  LOG("HAIntranuke2015", pINFO) << "DelRPion    = " << fDelRPion;
  LOG("HAIntranuke2015", pINFO) << "DelRNucleon = " << fDelRNucleon;
  LOG("HAIntranuke2015", pINFO) << "HadStep     = " << fHadStep << " fermi";
  LOG("HAIntranuke2015", pINFO) << "EPreEq      = " << fHadStep << " fermi";
