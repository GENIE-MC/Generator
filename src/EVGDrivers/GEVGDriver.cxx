...
{
  LOG("GEVGDriver", pINFO) << "Configuring a GEVGDriver object";
...
double GEVGDriver::SumCrossSection(const TLorentzVector & nup4)
{
  ...
  for(evgliter = fEvGenList->begin();
     ...
     for(intliter = ilst->begin(); intliter != ilst->end(); ++intliter) {
         ...
         SLOG("GEVGDriver", pINFO) << "Interaction = " << interaction->AsString();
         ...
         SLOG("GEVGDriver", pINFO)
            << "Cross Section = " << xsec << " / Running Sum = " << xsec_sum;
...
double GEVGDriver::MaxCrossSection(const TLorentzVector & nup4)
{
  ...
  for(evgliter = fEvGenList->begin();
     ...
     for(intliter = ilst->begin(); intliter != ilst->end(); ++intliter) {
         ...
         SLOG("GEVGDriver", pINFO)
            << "Cross Section = " << xsec << " / Current max = " << max_xsec;
     } // loop over interaction that can be generated by this generator
  ...
  for(evgliter = fEvGenList->begin();
     ...
     for(intliter = ilst->begin(); intliter != ilst->end(); ++intliter) {
         ...
         if(!spl_exists) {
             ...
             LOG("GEVGDriver", pINFO) << "Spline is already loaded - skipping";
         }
