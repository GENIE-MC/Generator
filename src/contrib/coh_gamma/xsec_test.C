...
void xsec_test( TString out_name = "coh_ncgamma.root" );
...
double GenieXSec(int nu_probe, int tgt, double Enu, double Enu_final, double theta_l, double theta_g, double phi_g);

void SigmaCosTh(const Interaction *i, const ARSXSec * xsec_alg, Kinematics * kine, SigVars sig_param); 


//__________________________________________________________________________
void xsec_test ( TString out_name )
{
...
double GenieXSec(int nu_probe, int tgt, double Enu, double Enu_final, double theta_l, double theta_g, double phi_g)
{
  ...
  AlgFactory * algf = AlgFactory::Instance();
  AlgId id("AlvarezRusoSalaCOHGammaPXSec","Default");
  const Algorithm * algo = algf->GetAlgorithm(id);
  const XSecAlgorithm * xsec_alg = dynamic_cast<const XSecAlgorithm*>( algo );
  
  Interaction * i = Interaction::COHNC( tgt, nu_probe, 22, Enu );
  
  const TLorentzVector P4_nu( 0., 0., E_nu, E_nu ) ;
  
  double m_t = i->InitState().Tgt().Mass() ;
  double cos_theta_g = cos( theta_g ) ;
