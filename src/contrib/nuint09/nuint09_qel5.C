...
//
// QEL.5:
// dSigma / dOmega_mu dKE_mu at E_nu = 0.5 and 1.0 GeV
//
// Inputs:
...
void nuint09_qel5(int isample)
{
  ...
  out_filename << label << ".qel_5.d2sig_dKEmudOmega.data";
  ...
  out_stream << "#  " << endl;
  out_stream << "# [QEL.5]:" << endl;
  out_stream << "#  dSigma / dOmega_mu dKE_mu at E_nu = 0.5 and 1.0 GeV" << endl;
  out_stream << "#  " << endl;
  ...
  out_stream << "#  |  KE(mu)  |  cos(theta_mu)  | sig(QELCC; Ev=0.5GeV)  | sig(QELCC; Ev=1.0GeV) | " << endl;
  ...
  // book histograms
  //
  TH2D * hst_d2sig_dKEmudOmg_qelcc_0500MeV =  new TH2D("hst_d2sig_dKEmudOmg_qelcc_0500MeV",
           "dsig/d2KEmudOmega, nu_mu QEL CC, Enu=0.5 GeV", nKEmu, KEmumin, KEmumax, ncosth, costhmin, costhmax);
  TH2D * hst_d2sig_dKEmudOmg_qelcc_1000MeV = new TH2D("hst_d2sig_dKEmudOmg_qelcc_1000MeV",
           "dsig/d2KEmudOmega, nu_mu QEL CC, Enu=1.0 GeV", nKEmu, KEmumin, KEmumax, ncosth, costhmin, costhmax);
  ...
  // normalize
  //
  double norm_qelcc_0500MeV = hst_d2sig_dKEmudOmg_qelcc_0500MeV -> Integral("width") * 2*TMath::Pi() / sig_qelcc_0500MeV;
  double norm_qelcc_1000MeV = hst_d2sig_dKEmudOmg_qelcc_1000MeV -> Integral("width") * 2*TMath::Pi() / sig_qelcc_1000MeV;

  if (norm_qelcc_0500MeV > 0) hst_d2sig_dKEmudOmg_qelcc_0500MeV -> Scale(1./norm_qelcc_0500MeV);
  if (norm_qelcc_1000MeV > 0) hst_d2sig_dKEmudOmg_qelcc_1000MeV -> Scale(1./norm_qelcc_1000MeV);

  for(int i = 1; i <= hst_d2sig_dKEmudOmg_qelcc_1000MeV->GetNbinsX(); i++) {
     for(int j = 1; j <= hst_d2sig_dKEmudOmg_qelcc_1000MeV->GetNbinsY(); j++) {

        double KEmu    = hst_d2sig_dKEmudOmg_qelcc_1000MeV -> GetXaxis() -> GetBinCenter(i);
        double costhmu = hst_d2sig_dKEmudOmg_qelcc_1000MeV -> GetYaxis() -> GetBinCenter(j);

        double d2sig_dKEmudOmg_qelcc_0500MeV = hst_d2sig_dKEmudOmg_qelcc_0500MeV -> GetBinContent(i,j);
        double d2sig_dKEmudOmg_qelcc_1000MeV = hst_d2sig_dKEmudOmg_qelcc_1000MeV -> GetBinContent(i,j);

        d2sig_dKEmudOmg_qelcc_0500MeV = TMath::Max(0., d2sig_dKEmudOmg_qelcc_0500MeV);
        d2sig_dKEmudOmg_qelcc_1000MeV = TMath::Max(0., d2sig_dKEmudOmg_qelcc_1000MeV);

        out_stream << setw(15) << KEmu 
                   << setw(15) << costhmu
                   << setw(15) << d2sig_dKEmudOmg_qelcc_0500MeV
                   << setw(15) << d2sig_dKEmudOmg_qelcc_1000MeV
                   << endl;
    }//costh
  }//E
