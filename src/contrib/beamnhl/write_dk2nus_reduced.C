...
int write_dk2nus_reduced(bool grid               = false,
			 bool debug              = false,
			 int job                 = 0,
			 std::string indir       = "",
			 std::string inString    = "",
...
void LoopEntries(TChain* cflux, TChain* dflux, bool grid, bool debug)
{
  ...
  //============================================================================
    ...
    std::cout << "\n  Will loop over " << dflux->GetEntries() << " meta entries.\n";
    
    for (Long64_t i=0; i < nentries; ++i ) {
      if (i % (nentries/100) == 0){
	std::cout << "entry " << i << " (" << i/(nentries/100) << "% done)" << std::endl;
	std::cerr << "\t" << Form( "%3d", i/(nentries/100) ) << " % done \r" << std::flush;
      }
      cflux->GetEntry(i);
      //dflux->GetEntry(i);
    ...
    // loop over dkmeta separately
    for (Long64_t i=0; i < dflux->GetEntries(); i++){
      dflux->GetEntry(i);
      FillMetaBranches(dkmeta);
    }
