...
// Return inclusive d^2sigma/dxdy cross-section (only neutrino CC for the time-being)
  ...
  if(fRESXSecModel != 0) {
    ...
    for(int ires=0; ires<kNRes; ires++) {
       ...
       double xsec = fRESXSecModel->XSec(res_int,kPSxyfE) / (1E-38 * units::cm2);
  ...
  if(fDISXSecModel != 0) {
    ...
    double xsec = fDISXSecModel->XSec(dis_int,kPSxyfE) / (1E-38 * units::cm2);
    xsec = TMath::Max(0., xsec);
    LOG("gvldtest", pINFO) 
        << "d2xsec_dxdy(CCDIS; " 
        << "E = " << E << " GeV, W = " << W << " GeV, Q2 = " << Q2 << " GeV^2"
  ...
  //
  double d2SigDISc_dxdy = 0; // neglect this for now 
