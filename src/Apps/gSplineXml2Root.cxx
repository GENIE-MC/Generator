...
void SaveGraphsToRootFile(void) {
  ...
  double de = (gInlogE)
                  ? (TMath::Log(gEmax) - TMath::Log(gEmin)) / (kNSplineP - 1)
                  : (gEmax - gEmin) / (kNSplineP - 1);
  double *e = new double[kNSplineP];
  for (int i = 0; i < kNSplineP; i++) {
    e[i] = (gInlogE) ? TMath::Exp(TMath::Log(gEmin) + i * de) : gEmin + i * de;
  }
...
void GetCommandLineArgs(int argc, char **argv) {
  ...
  // not yet//LOG("gspl2root", pINFO) << "  Keep spline knots  = " <<
  // (gKeepSplineKnots?"true":"false");
}
//____________________________________________________________________________
void PrintSyntax(void) {
